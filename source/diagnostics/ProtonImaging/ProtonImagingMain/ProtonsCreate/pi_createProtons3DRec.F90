!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/ProtonsCreate/pi_createProtons3DRec
!!
!! NAME
!!
...
subroutine pi_createProtons3DRec (blockCount, blockList, timeSimulation)
  ...
  use pi_interface,        ONLY : pi_recordProtonOnScreen,    &
...
!
            do while (nProtonsCapsule > 0)
               ...
               if (grainLevel > 0) then
                   ...
                   nProtons = min (nProtonsCapsule, nCircle)

                   pi_xSphere (1:nProtons) = grainLocalX                             !
                   pi_ySphere (1:nProtons) = grainLocalY                             ! same grain local origin
                   pi_zSphere (1:nProtons) = grainLocalZ                             !
               else
               ...
               if (nProtons == 0 .and. nProtonsCapsule > 0) then
                   call Driver_abortFlash ("pi_createProtons3DRec: Beam still has protons, but 0 to process!")
               end if
...
!
!
!     ...Catch the situation in which the proton does not hit the domain. If the user has
!        specified to flag this situation, the program is aborted. Otherwise, we give this
!        proton a position (on the capsule) and a velocity direction and record it directly
!        on the screen.
!
!
                  if (protonMissesDomain) then

                      if (pi_flagDomainMissingProtons) then
                          call Driver_abortFlash("pi_createProtons3DRec: proton does not hit domain boundary!")
                      else
                          posX = Cx                                  ! take initital position
                          posY = Cy                                  ! of these protons on
                          posZ = Cz                                  ! the capsule
                          ...
                          dgJv = 0.0                                 !
                          dgKx = 0.0                                 ! diagnostics set to zero
                          dgKy = 0.0                                 !
                          dgKz = 0.0                                 !
                          ...
                          cycle                                      ! take next proton in capsule
                      end if
                  end if
!
!
                  ...
                  do block = 1, blockCount
                     ...
                     if (inBlock) then
                         ...
                         if (ignoreBoundary .or. .not.protonReflects) then
                             ...
                             dgJv = 0.0                                 !
                             dgKx = 0.0                                 ! initialize diagnostics
                             dgKy = 0.0                                 !
                             dgKz = 0.0                                 !
!
!
...
!
                             ...
                             pi_protons (PROTON_DGJV,pi_protonCount) = dgJv
                             pi_protons (PROTON_DGKX,pi_protonCount) = dgKx
                             pi_protons (PROTON_DGKY,pi_protonCount) = dgKy
                             pi_protons (PROTON_DGKZ,pi_protonCount) = dgKz

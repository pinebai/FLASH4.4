!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/ProtonsTrace/pi_traceBlockProtons3DRec
!!
!! NAME
!!
...
!
!
!     ...Find the indices (i,j,k) of the cell where the proton currently is inside
!        the block. Make sure that this cell actually belongs to the block.
!
!
     ...
     if (protonOutOfBlock) then
         call Driver_abortFlash ('[pi_traceBlockProtons3DRec] ERROR: proton found in out of block cells')
     end if
!
!
...
!
!
!     ...The proton is in the (i,j,k) cell for sure. Make sure the proton is properly
!        nudged into the cell.
!
!
...
!
!
!     ...If the time spent in the domain exceeds the curent stepping time, add this proton
!        to the disk protons, remove the proton from the active domain list and take next
!        proton.
!
!
        if (pi_timeResolvedProtonImaging .and. time >= pi_timeStep) then

            !$omp critical (setDiskProtonCount)

                  pi_diskProtonCount = pi_diskProtonCount + 1

                  pi_diskProtons (PROTON_POSX, pi_diskProtonCount) = posX
                  pi_diskProtons (PROTON_POSY, pi_diskProtonCount) = posY
                  pi_diskProtons (PROTON_POSZ, pi_diskProtonCount) = posZ
                  pi_diskProtons (PROTON_VELX, pi_diskProtonCount) = velX
                  pi_diskProtons (PROTON_VELY, pi_diskProtonCount) = velY
                  pi_diskProtons (PROTON_VELZ, pi_diskProtonCount) = velZ
                  pi_diskProtons (PROTON_TAGS, pi_diskProtonCount) = pi_protons (PROTON_TAGS,proton)
                  pi_diskProtons (PROTON_BEAM, pi_diskProtonCount) = pi_protons (PROTON_BEAM,proton)
                  pi_diskProtons (PROTON_DETC, pi_diskProtonCount) = pi_protons (PROTON_DETC,proton)
                  pi_diskProtons (PROTON_DGJV, pi_diskProtonCount) = dgJv
                  pi_diskProtons (PROTON_DGKX, pi_diskProtonCount) = dgKx
                  pi_diskProtons (PROTON_DGKY, pi_diskProtonCount) = dgKy
                  pi_diskProtons (PROTON_DGKZ, pi_diskProtonCount) = dgKz

            !$omp end critical (setDiskProtonCount)

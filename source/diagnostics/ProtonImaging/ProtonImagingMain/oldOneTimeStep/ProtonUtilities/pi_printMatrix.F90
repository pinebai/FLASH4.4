!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/oldOneTimeStep/ProtonUtilities/pi_printMatrix
!!
!! NAME
!!
!!  pi_printMatrix
...
!
  if (rowMaxPrint > rowMaxMatrix .or. rowMinPrint < rowMinMatrix) then
      call Driver_abortFlash ("pi_printMatrix: Matrix row printing range out of bounds")
  end if

  if (colMaxPrint > colMaxMatrix .or. colMinPrint < colMinMatrix) then
      call Driver_abortFlash ("pi_printMatrix: Matrix column printing range out of bounds")
  end if

  if (rowMinPrint > rowMaxPrint .or. colMinPrint > colMaxPrint) then
      call Driver_abortFlash ("pi_printMatrix: No matrix printing range!")
  end if
!
!

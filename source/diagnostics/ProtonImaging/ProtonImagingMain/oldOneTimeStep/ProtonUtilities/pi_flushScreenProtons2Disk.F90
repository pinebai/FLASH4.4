!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/oldOneTimeStep/ProtonUtilities/pi_flushScreenProtons2Disk
!!
!! NAME
!!
...
subroutine pi_flushScreenProtons2Disk ()
  ...
  use pi_interface,        ONLY : pi_write2DetectorFile
...
!
  if (pi_globalMe == MASTER_PE) then
      ...
      pi_screenProtonCount = offset   ! this is the new final (total) screen proton count on the master

      if (pi_screenProtonCount > pi_maxProtonCount) then
          call Driver_abortFlash ('[pi_flushScreenProtons2Disk] ERROR: # of screen protons > storage size')
      end if
...
!
!
!     ...Process all screen protons on the master processor. Distribute (sort) screen protons into
!        their respective buckets and empty them when appropriate.
!
!
  if (pi_globalMe == MASTER_PE) then
      ...
      if (pi_screenProtonDiagnostics) then
          ...
          do proton = 1,pi_screenProtonCount
             ...
             if (bucketCount == pi_screenProtonBucketSize) then
                 ...
                 call pi_write2DetectorFile (detector,                                                    &
                                             recordCount,                                                 &
                                             bucketCount,                                                 &
                                             pi_screenProtonBuckets (1:recordCount,1:bucketCount,detector))
...
!
!
!     ...Empty remaining unfull buckets (if any) and reset screen proton counter on master
!        processor as well.
!
!
...
!
!
!     ...Reset screen proton counter on all processors.
!
!

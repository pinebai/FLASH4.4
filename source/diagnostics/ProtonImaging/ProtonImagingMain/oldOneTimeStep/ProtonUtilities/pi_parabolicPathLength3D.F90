!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/oldOneTimeStep/ProtonUtilities/pi_parabolicPathLength3D
!!
!! NAME
!!
...
real function pi_parabolicPathLength3D (vx,vy,vz,ax,ay,az,T)
  ...
  use ProtonImaging_data, ONLY : pi_unitRoundoff
...
!
!
!     ...Check the case present and calculate the 3D parabolic path length.
!
!
  ...
  if (aT * pi_unitRoundoff >= v + v) then
      L = 0.5 * aT * T
  else if (aT <= pi_unitRoundoff * (v + v)) then
      L = v * T
  else
      d = vx * ax + vy * ay + vz * az
      c = d / (a * v)
      ...
      if (c == 1.0) then
          ...
          X = aT / v
          d = sqrt (X * X + (X + X) * c + 1.0)
          d = (c + X) * (d - 1.0) + (1.0 - c * c) * log ((c + X + d)/(c + 1.0))
          d = 1.0 + d / X
          L = 0.5 * v * T * d
      end if
  end if

  pi_parabolicPathLength3D = L
!
!
!     ...Ready!
!

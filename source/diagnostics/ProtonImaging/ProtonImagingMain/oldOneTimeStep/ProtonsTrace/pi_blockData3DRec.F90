!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/oldOneTimeStep/ProtonsTrace/pi_blockData3DRec
!!
!! NAME
!!
!!  pi_blockData3DRec
...
#include "Flash.h"
  ...
  real    :: factorB, factorX, factorY, factorZ
!
!
!     ...Get the electric field and the boundary indicator.
!
!
  if (pi_ignoreElectricalField) then
      ...
      do j = jminBlock,jmaxBlock
      do i = iminBlock,imaxBlock
         pi_cellEfield (1,i,j,k) = blockData (ELEX_VAR,i,j,k)
         pi_cellEfield (2,i,j,k) = blockData (ELEY_VAR,i,j,k)
         pi_cellEfield (3,i,j,k) = blockData (ELEZ_VAR,i,j,k)
         pi_cellBoundary (i,j,k) = blockData (BDRY_VAR,i,j,k)
      enddo
      enddo
      enddo

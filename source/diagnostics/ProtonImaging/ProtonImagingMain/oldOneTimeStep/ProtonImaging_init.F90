!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/oldOneTimeStep/ProtonImaging_init
!!
!! NAME
!!  
!!  ProtonImaging_init
...
!
  ...
  call RuntimeParameters_get ("pi_IOaddBeamCapsules",           pi_IOaddBeamCapsules         )
...
!        be aligned along specific beams.
!
!
  call pi_setupBeams         ()
...
!
!
!     ...Prepare the format for writing lines of data to the detector(s). Set the screen
!        proton counter to zero. Since the screen protons accumulate with possible calls
!        to the proton imaging routine during a simulation, this has to be set here.
!        Allocate the needed arrays on the master processor to be used for flushing screen
!        protons to disk (allocate single sized arrays on the other nodes to avoid

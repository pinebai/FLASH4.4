!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/ProtonImaging_data
!!
!! NAME
!!
!!  ProtonImaging_data
...
!!   pi_currentStepNumber           (G) : Stores the current step number of the simulation
...
!!   pi_detectorFilesID             (C) : Will hold the detector files ID number
!!   pi_detectorFilesName           (C) : Will hold the detector files name
...
!!   pi_diskProtonCount             (C) : Disk proton count on each processor at any stage of the calculation
!!   pi_diskProtonCountOffsets      (C) : Disk proton count offsets info for master processor
!!   pi_diskProtonCountProcs        (C) : Disk proton count for each processor info for master processor
!!   pi_diskProtonNewFileID         (C) : Holds the new disk proton file ID number
!!   pi_diskProtonNewFileName       (C) : Holds the new disk proton file name
!!   pi_diskProtonNewFileNbatches   (C) : Number of disk proton batches on the new disk proton file at any stage
!!   pi_diskProtonOldFileID         (C) : Holds the old disk proton file ID number
!!   pi_diskProtonOldFileName       (C) : Holds the old disk proton file name
!!   pi_diskProtonOldFileNbatches   (C) : Number of disk proton batches on the old disk proton file at any stage
!!   pi_diskProtons                 (C) : Disk protons that do not leave the domain -> will be written to disk
...
!!   pi_globalDiskProtonCount       (C) : Global disk proton count at any stage of the calculation
!!   pi_globalMe                    (C) : Rank of global processor
!!   pi_globalNumProcs              (C) : Number of processors of the global communicator
!!   pi_globalProtonCount           (C) : Global proton count at any stage of the calculation
!!   pi_globalScreenProtonCount     (C) : Global screen proton count at any stage of the calculation
...
!!   pi_monitorFileName             (C) : File name for monitoring proton imaging progress (useful for master only)
!!   pi_monitorFileUnit             (C) : File unit # for monitoring proton imaging progress (useful for master only)
!!   pi_mpiDiskProtonType           (C) : Disk proton type structure for mpi communications
...
!!   pi_screenProtonBuckets         (C) : Screen proton buckets (:,:,:) -> (properties, count, detector)
...
!!   pi_timeResolvedProtonImaging   (R) : If true, time resolved proton imaging over many timesteps is invoked.
!!   pi_timeStep                    (C) : The current time step duration
...
# include "GridParticles.h"
  ...
  character (len = MAX_STRING_LENGTH), save :: pi_diskProtonNewFileName
  character (len = MAX_STRING_LENGTH), save :: pi_diskProtonOldFileName
  character (len = MAX_STRING_LENGTH), save :: pi_monitorFileName
  ...
  logical, save :: pi_timeResolvedProtonImaging
  ...
  integer, save :: pi_diskProtonCount
  integer, save :: pi_diskProtonNewFileID
  integer, save :: pi_diskProtonNewFileNbatches
  integer, save :: pi_diskProtonOldFileID
  integer, save :: pi_diskProtonOldFileNbatches
  integer, save :: pi_globalComm
  integer, save :: pi_globalDiskProtonCount
  integer, save :: pi_globalMe
  integer, save :: pi_globalNumProcs
  integer, save :: pi_globalProtonCount
  integer, save :: pi_globalScreenProtonCount
  ...
  integer, save :: pi_monitorFileUnit
  integer, save :: pi_mpiDiskProtonType
  ...
  character (len = MAX_STRING_LENGTH), save, allocatable :: pi_detectorFilesName        (:)
  type (detectorType),                 save, allocatable :: pi_detectors                (:)
  integer,                             save, allocatable :: pi_diskProtonCountOffsets   (:) ! useful for master
  integer,                             save, allocatable :: pi_diskProtonCountProcs     (:) ! useful for master
  ...
  integer,                             save, allocatable :: pi_screenProtonBucketCount  (:) ! useful for master
  integer,                             save, allocatable :: pi_screenProtonCountOffsets (:) ! useful for master
  integer,                             save, allocatable :: pi_screenProtonCountProcs   (:) ! useful for master
  ...
  real,                                save, allocatable :: pi_diskProtons              (:,:)
  ...
  real,                                save, allocatable :: pi_screenProtonBuckets      (:,:,:)

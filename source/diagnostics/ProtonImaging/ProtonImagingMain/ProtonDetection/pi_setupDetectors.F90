!!****if* source/diagnostics/ProtonImaging/ProtonImagingMain/ProtonDetection/pi_setupDetectors
!!
!! NAME
!!
!!  pi_setupDetectors
...
#include "constants.h"
  ...
  real    :: pinholeDist2Det, pinholeRadius
...
!
  do detector = 1, pi_numberOfDetectors
     ...
     write (parameterString,'(a,i0)') "pi_detectorPinholeDist2Det_", detector
     call RuntimeParameters_get (parameterString, pinholeDist2Det)

     write (parameterString,'(a,i0)') "pi_detectorPinholeRadius_", detector
     call RuntimeParameters_get (parameterString, pinholeRadius)
...
!
!  
     if (pinholeRadius > 0.0 .and. pinholeDist2Det < 0.0) then
         write (*,*) ' Detector # , pinhole radius            = ',detector, pinholeRadius
         write (*,*) ' Detector # , pinhole detector distance = ',detector, pinholeDist2Det
         call Driver_abortFlash ("pi_setupDetectors: Pinhole specified, but no distance from detector!")
     end if

     if (pinholeDist2Det > 0.0 .and. pinholeRadius < 0.0) then
         write (*,*) ' Detector # , pinhole radius            = ',detector, pinholeRadius
         write (*,*) ' Detector # , pinhole detector distance = ',detector, pinholeDist2Det
         call Driver_abortFlash ("pi_setupDetectors: No pinhole size, but distance to detector given!")
     end if
     ...
     if (      (sideTiltingAxis /= 'x') &
         ...
         .and. (sideTiltingAxis /= 'z') ) then
          ...
          call Driver_abortFlash ("pi_setupDetectors: invalid side tilting axis!")
     end if
...
!  
     ...
     pi_detectors (detector) % pinholeDist2Det           = pinholeDist2Det              ! in cm
     pi_detectors (detector) % pinholeRadius             = pinholeRadius                ! in cm
...
!
!
!   ...Allocate detector file ID and name array for further reference.
!
!
  allocate (pi_detectorFilesID   (1:pi_numberOfDetectors))
  allocate (pi_detectorFilesName (1:pi_numberOfDetectors))
!
!

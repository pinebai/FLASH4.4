...
#include "Flash.h"
  ...
  !! stuff that's not in the basic specialized stub
  ! local variables
  ...
  real    :: tempRho
  ! array to hold coordinate information for the block
      ...
      if(mask(ivar)) then
        ...
        if(face==LOW) then ! apply BC on the bottom boundary at x=xmin
           ...
           case(USER_DEFINED)
              ...
              ! Temporary (using outflow)
              ! ###############
              ! ###############
              !print*,'--- BC is user_defined, but just using outflow (min) ---'
              do i = 1,guard
                 regionData(i,1:je,1:ke,ivar)= regionData(guard+1,1:je,1:ke,ivar)
              end do
              ! ###############
              ...
              do i = 1,guard
                 do j=1,je
                    if (gridDataStruct==CENTER) then
                       !if (ivar == DENS_VAR) regionData(i,j,1:ke,ivar) = regionData(guard+1,j,1:ke,ivar)
                       !if (ivar == VELX_VAR) regionData(i,j,1:ke,ivar) = regionData(guard+1,j,1:ke,ivar)
                       !if (ivar == VELY_VAR) regionData(i,j,1:ke,ivar) = regionData(guard+1,j,1:ke,ivar)
                       !if (ivar == VELZ_VAR) regionData(i,j,1:ke,ivar) = regionData(guard+1,j,1:ke,ivar)
                       !if (ivar == PRES_VAR) regionData(i,j,1:ke,ivar) = regionData(guard+1,j,1:ke,ivar)
                       !if (ivar == GAMC_VAR) regionData(i,j,1:ke,ivar) = regionData(guard+1,j,1:ke,ivar)
           ...
           case default
              ...
              !print*,'boundary (lower face) is unsupported by Gric_bcApplyToRegionSpecialized, passing to standard'
              applied = .false.
           end select
           ...
           case(USER_DEFINED)
              ...
              ! ###############
              ! ###############
              !print*,'--- BC is user_defined, but just using outflow (max) ---'
              k=guard
              do i = 1,guard
                 regionData(k+i,1:je,1:ke,ivar)= regionData(k,1:je,1:ke,ivar)
              end do
           ...
           case default
              !print*,'boundary is',bcType
              !print*,'boundary (upper face) is unsupported by Gric_bcApplyToRegionSpecialized, passing to standard'

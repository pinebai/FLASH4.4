!!****if* source/Simulation/SimulationMain/radflaHD/BondiAccretion/Simulation_init
!!
!! NAME
!!
!!  Simulation_init
...
subroutine Simulation_init()
  ...
  use Eos_interface
  use PhysicalConstants_interface
...
!  integer :: var_key(max_stored_vars)
  ...
  call Eos_getParameters    ( smallE2=sim_smallEele)
  ...
  call RuntimeParameters_get("sim_TradInitScaleFactor",sim_TradInitScaleFactor)
  call RuntimeParameters_get("sim_initializeAnalytic",sim_initializeAnalytic)
  call RuntimeParameters_get("sim_accretionRate",sim_accretionRate)
  ...
  call RuntimeParameters_get("ptmass",sim_usedPointMass)
  call RuntimeParameters_get("op_transConst",sim_transOpacConst)

  ! Store physical constants:
  call PhysicalConstants_get("Stefan-Boltzmann",sim_sb)
  call PhysicalConstants_get("speed of light",sim_speedlt)
  sim_speedlt3 = sim_speedlt / 3.0
  call PhysicalConstants_get("Stefan-Boltzmann",sim_radconst)
  sim_radconst = 4.0 * sim_radconst / sim_speedlt

  call PhysicalConstants_get("Newton",gravConst)


  sim_massLoss = sim_massLoss * 2.0e33 / 31556926.

  ! Compute Bondi radius once and for all for this simulation,
  ! taking account of the mass of the central gravitating object
  ! as well as the expected (analytical) radiation pressure.

  GM = gravConst * sim_usedPointMass
  sim_luminosity = 1.6e5 * 3.99e33

  ! Fraction of Eddington luminosity with which the point mass is
  ! radiating, according to Krumholz et al.
  if (sim_useMGD) then
     eddiFract = sim_transOpacConst * sim_luminosity / (4.*PI*GM*sim_speedlt)
  else
     eddiFract = 0.0
  end if

  sim_soundSpeedInf = 1.3e7
  sim_bondiRadius = (1.0-eddiFract) * GM / (sim_soundSpeedInf**2)

  if (sim_meshME==MASTER_PE) then
     print*,' *** f_Eddi      :', eddiFract,      ' ***'
     print*,' *** Bondi Radius:', sim_bondiRadius,' ***'
  end if

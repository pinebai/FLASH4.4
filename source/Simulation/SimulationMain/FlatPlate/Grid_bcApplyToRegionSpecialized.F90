!!****if* source/Simulation/SimulationMain/FlatPlate/Grid_bcApplyToRegionSpecialized
!!
!!
!! NAME
...
#include "Flash.h"
  ...
  real :: soundSpeed,totalVel,velx_zone,vely_zone


  !! Set this true if user boundary condition is needed
  if (axis .ne. KAXIS) then
     applied = .true.
  else
     applied = .false.
  endif
           ...
           if(face==LOW) then ! apply BC on the bottom boundary at x=xmin
              ...
              case(USER_DEFINED)
                 ...
                 do i = 1,guard
                    do j=1,je
                       if (gridDataStruct==CENTER) then
                          if (ivar == DENS_VAR) regionData(i,j,1:ke,ivar) = sim_rhoAmbient
                          if (ivar == VELX_VAR) regionData(i,j,1:ke,ivar) = sim_windVelx
                          if (ivar == VELY_VAR) regionData(i,j,1:ke,ivar) = sim_windVely
                          if (ivar == VELZ_VAR) regionData(i,j,1:ke,ivar) = sim_windVelz
                          if (ivar == PRES_VAR) regionData(i,j,1:ke,ivar) = sim_pAmbient
                          ...
                          if (ivar == EINT_VAR) regionData(i,j,1:ke,ivar) = &
                               sim_pAmbient/(sim_rhoAmbient*(sim_gamma-1.0))
                          if (ivar == ENER_VAR) regionData(i,j,1:ke,ivar) = &
                               sim_pAmbient/(sim_rhoAmbient*(sim_gamma-1.0)) &
                               + 0.5*(sim_windVelx**2 + sim_windVely**2)
                          if (ivar == BDRY_VAR) regionData(i,j,1:ke,ivar) = -1.0
                       endif
                    end do
                 end do
              case default

!!****if* source/Simulation/SimulationMain/CSM_RAD/Simulation_initBlock
!!
!! NAME
!! 
             ...
             ! print*,'DO I MAKE IT HERE?'
              ...
              if(sim_shellcond) then
                 if (xCenter(i) <= xzn(n1d_total)) then
                    ...
                    if(xCenter(i) < sim_rinner)then
                       solnData(1:NUNK_VARS,i,j,k) = model_1d(n1d_total,1:NUNK_VARS)
                       solnData(TEMP_VAR,i,j,k) = sim_shelltempfac*model_1d(n1d_total,TEMP_VAR)
                       solnData(VELX_VAR,i,j,k) = sim_windVel
                       solnData(DENS_VAR,i,j,k) = sim_massLoss / (4.0 * PI * xCenter(i)**2 * sim_windVel)
                       solnData(DENS_VAR,i,j,k) = max(solnData(DENS_VAR,i,j,k), sim_smlrho)
                    elseif((xCenter(i) >= sim_rinner).and.(xCenter(i) <= sim_router))then
                       solnData(1:NUNK_VARS,i,j,k) = model_1d(n1d_total,1:NUNK_VARS)
                       solnData(VELX_VAR,i,j,k) = sim_windVel
                       solnData(DENS_VAR,i,j,k) = sim_shelldens*(sim_rinner/xCenter(i))**2
                       !solnData(DENS_VAR,i,j,k) = max(solnData(DENS_VAR,i,j,k), sim_smlrho)
                       solnData(TEMP_VAR,i,j,k) = sim_shelltempfac*model_1d(n1d_total,TEMP_VAR)
                    elseif(xCenter(i) > sim_router)then
              ...
              !! PRESSURE/THERMAL BOMB SETUP
              !!if((xCenter(i) >= sim_bombRadIn).and.(xCenter(i) <= sim_bombRad))then
              !if(xCenter(i) <= sim_bombRad)then
              !1. USING DENSITY AND INTERNAL ENERGY AS INPUTS FOR THE EOS
              !!  eint = sim_ExpEner/(sim_coremass * 1.99e+33)
              !!  eint = solnData(EINT_VAR,i,j,k) + eint
              !!  solnData(EINT_VAR,i,j,k) = eint
              !2. USING DENSITY AND PRESSURE AS INPUTS FOR THE EOS
              !pres = (1./3.)*(sim_ExpEner/(sim_coremass * 1.99e+33))
              !pres = solnData(PRES_VAR,i,j,k) +  pres
              !solnData(PRES_VAR,i,j,k) = pres
              !3. USING DENSITY AND TEMPERATURE AS INPUTS FOR THE EOS/DELTA
              !temp = 1.2d10
              !temp = (sim_ExpEner/(arad*(4./3.)*pi*(sim_bombRad-sim_bombRadIn)**3))**0.25
              !!  eosData(EOS_DENS) = solnData(DENS_VAR,i,j,k)
              !!  eosData(EOS_EINT) = solnData(EINT_VAR,i,j,k)
              ...
              !temp = solnData(TEMP_VAR,i,j,k) + temp
              !solnData(TEMP_VAR,i,j,k) = temp
              !4. USING DENSITY AND TEMPERATURE AS INPUTS FOR THE EOS/STAND.
              !temp = (sim_ExpEner/(arad*(4./3.)*pi*(xCenter(i))**3))**0.25
              !temp = solnData(TEMP_VAR,i,j,k) + temp
              !solnData(TEMP_VAR,i,j,k) = temp

!!****if* source/physics/Eos/EosMain/multiTemp/Helmholtz/eos_vecAlloc
!!
!! NAME
!!
!!  eos_vecAlloc
...
!UNUSED  allocate(xniRow(vecLen))
  ...
  allocate(cvIonRow(vecLen))
  allocate(cvEleRow(vecLen))
  allocate(gamcRow(vecLen))

  allocate(tempRadRow(vecLen))
  allocate(tempIonRow(vecLen))
  allocate(tempEleRow(vecLen))
  allocate(eCompRow(EOSCOMP_NUM_COMPONENTS,vecLen))
  allocate(pCompRow(EOSCOMP_NUM_COMPONENTS,vecLen))

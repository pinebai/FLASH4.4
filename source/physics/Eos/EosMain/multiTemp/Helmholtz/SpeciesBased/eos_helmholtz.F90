!!****if* source/physics/Eos/EosMain/multiTemp/Helmholtz/SpeciesBased/eos_helmholtz
!!
!! NAME
!!
!! eos_helmholtz
...
!!$  logical, dimension(EOS_VARS+1:EOS_NUM) :: maskInternal
  ...
  case(MODE_DENS_TEMP_ION,MODE_DENS_TEMP_ELE,MODE_DENS_TEMP_RAD)
     ...
     if(maskPtr(EOS_EINTION)) eosData(eintIon+1:eintIon+vecLen)=eCompRow(EOSCOMP_ION,1:vecLen)
     if(maskPtr(EOS_EINTELE)) eosData(eintEle+1:eintEle+vecLen)=eCompRow(EOSCOMP_ELE,1:vecLen)
...
!!$        eosData(eintEle+1:eintEle+vecLen) = esaveRow(1:vecLen)
     ...
     ewantRow(1:vecLen) = eosData(eintIon+1:eintIon+vecLen)   ! store desired ion energy
     call eos_newtonRaphson(vecLen, MODE_DENS_EI_ION, .TRUE., maskPtr,componentMask)
     eosData(tempIon+1:tempIon+vecLen)=tempRow(1:vecLen)
...
!!$        eosData(eintIon+1:eintIon+vecLen) = esaveRow(1:vecLen)
        ...
        if(maskPtr(EOS_EINTION)) eosData(eintIon+1:eintIon+vecLen) = etotRow(1:vecLen)
     end if

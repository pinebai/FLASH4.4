!!****if* source/physics/Eos/EosMain/multiTemp/Helmholtz/Ye/eos_helmholtz
!!
!! NAME
!!
!! eos_helmholtz
...
subroutine eos_helmholtz(mode,vecLen,eosData,massFrac,mask)
  ...
  use Eos_data, ONLY: eos_smallT, eos_tol, eos_maxNewton, &
       eos_combinedTempRule, eos_forceConstantInput, eos_meshMe
...
#include "Eos_components.h"
  ...
  case(MODE_DENS_TEMP_EQUI)
     ...
     eosData(pres+1:pres+vecLen)      = ptotRow(1:vecLen)
     if(maskInternal(EOS_PRESION)) eosData(presIon+1:presIon+vecLen)= pCompRow(EOSCOMP_ION,1:vecLen)
     if(maskInternal(EOS_PRESELE)) eosData(presEle+1:presEle+vecLen)= pCompRow(EOSCOMP_ELE,1:vecLen)
     if(maskInternal(EOS_PRESRAD)) eosData(presRad+1:presRad+vecLen)= pCompRow(EOSCOMP_RAD,1:vecLen)
     eosData(eint+1:eint+vecLen)      = etotRow(1:vecLen)
     if(maskInternal(EOS_EINTION)) eosData(eintIon+1:eintIon+vecLen)= eCompRow(EOSCOMP_ION,1:vecLen)
     if(maskInternal(EOS_EINTELE)) eosData(eintEle+1:eintEle+vecLen)= eCompRow(EOSCOMP_ELE,1:vecLen)
     if(maskInternal(EOS_EINTRAD)) eosData(eintRad+1:eintRad+vecLen)= eCompRow(EOSCOMP_RAD,1:vecLen)
     eosData(gamc+1:gamc+vecLen)      = gamcRow(1:vecLen)
     eosData(entr+1:entr+vecLen)      = stotRow(1:vecLen)
  ...
  case(MODE_DENS_TEMP_COMP,MODE_DENS_TEMP_GATHER)
     ...
     if(mode == MODE_DENS_TEMP_GATHER)then
        if(eos_combinedTempRule==0) then
           call eos_newtonRaphson(vecLen, MODE_DENS_EI, .TRUE., maskInternal,cMask=(/1,1,1/))
        endif
     endif
  ...
  case(MODE_DENS_TEMP_ION,MODE_DENS_TEMP_ELE,MODE_DENS_TEMP_RAD)
     ...
     call eos_helm(1,vecLen,maskInternal,componentMask)
  ...
  case(MODE_DENS_EI_ION,MODE_DENS_EI_ELE,MODE_DENS_EI_RAD)
     ...
     call eos_newtonRaphson(vecLen, mode, .TRUE., maskInternal,componentMask)


  case(MODE_DENS_EI_COMP,MODE_DENS_EI_ALL,MODE_DENS_EI_GATHER)
     ...
     ewantRow(1:vecLen) = eosData(eintRad+1:eintRad+vecLen)   ! store desired radiation energy
     call eos_newtonRaphson(vecLen, MODE_DENS_EI_RAD, .TRUE., maskInternal,componentMask)
     eosData(tempRad+1:tempRad+vecLen)=tempRow(1:vecLen)
     if(maskInternal(EOS_PRESRAD)) eosData(presRad+1:presRad+vecLen)=ptotRow(1:vecLen)
     if (eos_forceConstantInput)  then
!!$        eosData(eintRad+1:eintRad+vecLen) = esaveRow(1:vecLen)
     else
        if(maskInternal(EOS_EINTRAD)) eosData(eintRad+1:eintRad+vecLen) = etotRow(1:vecLen)
     end if
     ...
     ewantRow(1:vecLen) = eosData(eintEle+1:eintEle+vecLen)   ! store desired electron energy
     call eos_newtonRaphson(vecLen, MODE_DENS_EI_ELE, .TRUE., maskInternal,componentMask)
     eosData(tempEle+1:tempEle+vecLen)=tempRow(1:vecLen)
     if(maskInternal(EOS_PRESELE)) eosData(presEle+1:presEle+vecLen)=ptotRow(1:vecLen)
     if (eos_forceConstantInput)  then
!!$        eosData(eintEle+1:eintEle+vecLen) = esaveRow(1:vecLen)
     else
        if(maskInternal(EOS_EINTELE)) eosData(eintEle+1:eintEle+vecLen) = etotRow(1:vecLen)
     end if

     componentMask(:) = (/0,0,0/)
     componentMask(EOSCOMP_ION) = 1
     tempRow(1:vecLen) = tempIonRow(1:vecLen)
     ewantRow(1:vecLen) = eosData(eintIon+1:eintIon+vecLen)   ! store desired ion energy
     call eos_newtonRaphson(vecLen, MODE_DENS_EI_ION, .TRUE., maskInternal,componentMask)
     eosData(tempIon+1:tempIon+vecLen)=tempRow(1:vecLen)
     if(maskInternal(EOS_PRESION)) eosData(presIon+1:presIon+vecLen)=ptotRow(1:vecLen)
     if (eos_forceConstantInput)  then
!!$        eosData(eintIon+1:eintIon+vecLen) = esaveRow(1:vecLen)
     else
        if(maskInternal(EOS_EINTION)) eosData(eintIon+1:eintIon+vecLen) = etotRow(1:vecLen)
     end if
     ...
     if (mode==MODE_DENS_EI_ALL) then
        ...
        call eos_newtonRaphson(vecLen, MODE_DENS_EI, .TRUE., maskInternal,componentMask)
     end if
     if (mode==MODE_DENS_EI_GATHER) then
        tempRow(1:vecLen) = eosData(temp+1:temp+vecLen)
        if (ANY(maskInternal((/EOS_PRESELE,EOS_PRESION,EOS_PRESRAD/)) .EQV. .FALSE.)) then
  ...
  case(MODE_DENS_PRES)
     ...
     componentMask(:) = 1
     call eos_newtonRaphson(vecLen, mode, .FALSE., maskInternal, componentMask)
  ...
  !! DEV: BEWARE CHEATING FOR NOW , THINKING FOR LATER
    ...
    if(mask(EOS_CVELE))then
        if(mask(EOS_DET)) then
           ...
           call Driver_abortFlash("[eos_helmholtz] cannot calculate C_VELE without DET.  Set mask appropriately.")
        end if
     end if
  !!
...
!!$  real,dimension(vecLen),intent(IN)    ::  XXwantRow
!!$  real,dimension(vecLen),intent(INOUT) ::  XXtotRow
!!$  real,dimension(vecLen),intent(OUT)   ::  dXXtRow

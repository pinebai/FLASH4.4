!!****if* source/physics/Eos/EosMain/multiTemp/MatRad3/Multigamma/eos_mgamma
!!
!!
!! NAME
!!
...
!! PARAMETERS
!!
!!     gammaEle     :  Ratio of specific heats for electron component;
!!                     the gamma for other matter components, i.e., ions,
!!                     is taken from the Multispecies database.
!!
!! EXAMPLE
!!
...
#ifdef EOS_FORCE_2T
  integer,parameter :: overallCMask(EOSCOMP_NUM_COMPONENTS) = (/0,1,0/)
#else
  integer,parameter :: overallCMask(EOSCOMP_NUM_COMPONENTS) = (/0,1,1/)
#endif
...
!     doUncoupledForAddtl = .TRUE.
     if(maskPtr(EOS_PRESELE)) &
          ...
          eosData(tempEle+ilo:tempEle+ihi) / eosData(abar+ilo:abar+ihi) * Zp
  ...
  case (MODE_DENS_TEMP_GATHER)
     ...
     eosData(eintIon+ilo:eintIon+ihi) = ggprodIon * eosData(tempEle+ilo:tempEle+ihi) &
          / eosData(abar+ilo:abar+ihi)
     eosData(eintEle+ilo:eintEle+ihi) = ggprodEle * eosData(tempEle+ilo:tempEle+ihi) &
          / eosData(abar+ilo:abar+ihi) * dynamicZ &
        + eosData(eintIon+ilo:eintIon+ihi)
     eosData(eintIon+ilo:eintIon+ihi) = 0.0
...
!!$        end if
        ...
        if(eos_combinedTempRule==0) then
           call eos_newtonRaphson(vecLen, MODE_DENS_EI, .TRUE., maskPtr)
        endif
...
!        eosData(gamc+ilo:gamc+ihi)=gamcRow(1:rowLen) !replace with expression based on component states!
  ...
  case (MODE_DENS_TEMP_EQUI)
     ...
     eosData(presEle+ilo:presEle+ihi) = eos_gasConstant * &
          eosData(dens+ilo:dens+ihi) * &
          eosData(temp+ilo:temp+ihi) / eosData(abar+ilo:abar+ihi) * Zp
     eosData(pres+ilo:pres+ihi) = eosData(presEle+ilo:presEle+ihi)
     ...
     if(maskPtr(EOS_EINTELE)) eosData(eintEle+ilo:eintEle+ihi)=eosData(eint+ilo:eint+ihi)
     ...
     if(maskPtr(EOS_EINTION)) eosData(eintIon+ilo:eintIon+ihi)=0.0
     eosData(entr+ilo:entr+ihi) = 0.0
...
!     eosData(gamc+ilo:gamc+ihi)=gamcRow(1:rowLen)  ! Set in prologue! / set in epilogue!
     ...
     if (mode == MODE_DENS_EI_SCATTER) then
        ...
        if(maskPtr(EOS_PRESION)) eosData(presIon+ilo:presIon+ihi)=0.0
        if(maskPtr(EOS_PRESELE)) eosData(presEle+ilo:presEle+ihi)=pCompRow(EOSCOMP_ION,1:rowLen)+pCompRow(EOSCOMP_ELE,1:rowLen)
        if(maskPtr(EOS_PRESRAD)) eosData(presRad+ilo:presRad+ihi)=pCompRow(EOSCOMP_RAD,1:rowLen)
        if(maskPtr(EOS_EINTION)) eosData(eintIon+ilo:eintIon+ihi)=0.0
        if(maskPtr(EOS_EINTELE)) eosData(eintEle+ilo:eintEle+ihi)=eCompRow(EOSCOMP_ION,1:rowLen)+eCompRow(EOSCOMP_ELE,1:rowLen)
...
!!$        ewantRow(1:vecLen) = eosData(eintRad+1:eintRad+vecLen)   ! store desired radiation energy
  ...
  case (MODE_DENS_EI_COMP)
     ...
     eosData(presEle+ilo:presEle+ihi) =      eosData(dens+ilo:dens+ihi) * &
          ( eosData(eintIon+ilo:eintIon+ihi)*gam1invIon &
           +eosData(eintEle+ilo:eintEle+ihi)* &
            (gam1invEle * dynamicZ + gam1invIon)/Zp )
...
!!     eosData(entr+ilo:entr+ihi) = 0.0
     ...
     if (.TRUE.) then
        ...
        do i=ilo,ihi
           if (eosData(presEle+i) .NE. 0.0) then
              eosData(presRad+i) = 0.0
              eosData(pres+i)       = eosData(presEle+i)
              call setCombinedGamc1(gamc,i)
           else if (eosData(presRad+i) > 0.0) then
              eosData(gamc+i)       = eos_gammaRad
           else
              eosData(gamc+i)       = eos_gammaEle
           end if
        end do
        eosData(eint+ilo:eint+ihi) = ( &
             eosData(eintEle+ilo:eintEle+ihi)+ &
     ...
     if(mask(EOS_DET))then
        ...
        if(useNRForAddtlCombined) then
           ...
           call averageVectRByPressureFraction(det, &
                eosData(eintEle+ilo:eintEle+ihi) / eosData(tempEle+ilo:tempEle+ihi), &
                4 * eosData(eintRad+ilo:eintRad+ihi) / max(eosData(tempRad+ilo:tempRad+ihi),eos_smallT))
        else
...
!        call Driver_abortFlash("[eos_mgamma] cannot calculate ETA in the multiTemp / Gamma implementation.")
     ...
     if(mask(EOS_CVION))then    !??? DUST HEAT CAPACITY HERE ???
...
!!$    print*,'setCGamc->',eosData(gamcCombined+ilo:gamcCombined+ihi)
  ...
  subroutine averageVectRByPressureFraction(combined,compEle,compRad)
    integer, intent(in) :: combined
    real, intent(in),dimension(1:rowLen) :: compEle,compRad
  ...
  !  that it's not worth the small storage save.
    ...
    do k = rowBegin, rowEnd
       ...
       print *, ' '
       print *, 'Newton-Raphson failed in routine multiTemp MatRad3 Multigamma Eos'

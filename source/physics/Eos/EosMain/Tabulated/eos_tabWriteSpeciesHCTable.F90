!!****if* source/physics/Eos/EosMain/Tabulated/eos_tabWriteSpeciesHCTable
!!
!! NAME
!!
...
!! SYNOPSIS
!!
!!  call eos_tabWriteSpeciesHCTable (integer (in) :: fileUnit,
!!                               integer (in) :: species)
!!
!! DESCRIPTION
!!
!!  Prints out the tabulated heat capacity data for the current species.
!!
!! ARGUMENTS
!!
...
!
  ...
  if (.NOT.associated(eos_allTab(species)%tg(EOS_TABVT_HC)%table)) then
      call Driver_abortFlash ('[eos_tabWriteSpeciesHCTable] ERROR: no HC data tables')
  end if
!
!
!   ...Get the current temperature and density grid.
!
  td => eos_allTab(species)%tg(EOS_TABVT_HC)%td
  nstepsTemperature = td%ntemp
  nstepsDensity     = td%ndens
!
!
!   ...Print out the HC table in columns of 10 for each group. 
!
!
...
#ifdef DEBUG_EOS
     call Logfile_stamp("Zero-size HC tata tables???")
#endif
     return !RETURN IMMEDIATELY
  end if


  do g = LBOUND(eos_allTab(species)%tg(EOS_TABVT_HC)%table,1), &
       UBOUND(eos_allTab(species)%tg(EOS_TABVT_HC)%table,1)


     tb => eos_allTab(species)%tg(EOS_TABVT_HC)%table(g)
     if (.NOT.associated(tb%table)) then
#ifdef DEBUG_EOS
        if (eos_meshMe==0) &
             print*,'eos_tabWriteSpeciesHCTable: .NOT.associated(tb%table) for g=', g,', species=',species
#endif
        cycle
     end if

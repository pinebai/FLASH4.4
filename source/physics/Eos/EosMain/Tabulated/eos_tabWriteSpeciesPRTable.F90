!!****if* source/physics/Eos/EosMain/Tabulated/eos_tabWriteSpeciesPRTable
!!
!! NAME
!!
...
!! SYNOPSIS
!!
!!  call eos_tabWriteSpeciesPRTable (integer (in) :: fileUnit,
!!                               integer (in) :: species)
!!
!! DESCRIPTION
!!
!!  Prints out the tabulated pressure data for the current species.
!!
!! ARGUMENTS
!!
...
!
  ...
  if (.NOT.associated(eos_allTab(species)%tg(EOS_TABVT_PR)%table)) then
      call Driver_abortFlash ('[eos_tabWriteSpeciesPRTable] ERROR: no PR data tables')
  end if
!
!
!   ...Get the current temperature and density grid.
!
  td => eos_allTab(species)%tg(EOS_TABVT_PR)%td
  nstepsTemperature = td%ntemp
  nstepsDensity     = td%ndens
!
!
!   ...Print out the PR table in columns of 10 for each group. 
!
!
...
#ifdef DEBUG_EOS
     call Logfile_stamp("Zero-size PR tata tables???")
#endif
     return !RETURN IMMEDIATELY
  end if


  do g = LBOUND(eos_allTab(species)%tg(EOS_TABVT_PR)%table,1), &
       UBOUND(eos_allTab(species)%tg(EOS_TABVT_PR)%table,1)


     tb => eos_allTab(species)%tg(EOS_TABVT_PR)%table(g)
     if (.NOT.associated(tb%table)) then
#ifdef DEBUG_EOS
        if (eos_meshMe==0) &
             print*,'eos_tabWriteSpeciesPRTable: .NOT.associated(tb%table) for g=', g,', species=',species
#endif
        cycle
     end if
